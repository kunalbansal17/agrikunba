...
/**
 * Predict Prices — Investor MVP (Pages Router)
 * Enhancements in this version:
 * 1) ⓘ Tooltips on key tiles (explainers)
 * 2) Language selector (EN + HI + MR + TE + TA + BN) with on-page translation
 * 3) Caselet section for investors
 * 4) Turmeric photo card (place an image at /public/images/turmeric.jpg)
 *
 * Drop this file in: /pages/predictPrices.jsx
 * Deps: tailwindcss, recharts, lucide-react
 */

// --------------------------- i18n -----------------------------------------
const translations = {
  en: {
    title: "Predict Prices — India",
    investor_mvp: "Investor MVP",
...
export default function PredictPrices() {
  ...
  const tickerItems = useMemo(
    () => markets.map((m) => `${commodity} • ${m.market}, ${m.state} • ${unit} ${fmtIN(m.price)}`),
  ...
  {fmtTimeIST(data?.updatedAt)}
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            return (
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-lg font-semibold">{unit} {fmtIN(delivered)}</div>
                </div>
        ...
        <Card title={<TitleWithInfo title={t("basis_heatmap")} info={t("i_basis")} />} subtitle={t("basis_sub")} right={<TrendingUp className="h-4 w-4 text-gray-400" />}>
          <div className="flex items-center justify-between text-sm">
            <div>
              ...
              <div className="text-lg font-semibold">{unit} {fmtIN(national.price)}</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-gray-500">Near futures</div>
              <div className="text-lg font-semibold">{unit} {fmtIN(data?.futures?.near ?? national.price)}</div>
            </div>
          </div>
          <div className="mt-2 text-sm">Basis: <b>{unit} {fmtIN(national.price - (data?.futures?.near ?? national.price))}</b> · Seasonal band ±{fmtIN(180)}</div>
        ...
        <Card title={<TitleWithInfo title={t("anomaly")} info={t("i_anomaly")} />} subtitle={t("anomaly_sub")} right={<TrendingUp className="h-4 w-4 text-gray-400" />}>
          ...
          <div className="mt-1 text-sm">Top gap: <b>{unit} {fmtIN(anomaly.topGap)}</b> ({anomaly.pair})</div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card title={t("national_price")} subtitle={t("national_sub")}>
          <div className="text-2xl font-semibold">{unit} {fmtIN(national.price)}</div>
          ...
          <div className="mt-4">
            <ResponsiveContainer width="100%" height={120}>
              <AreaChart data={markets[0]?.series || []} margin={{ left: 0, right: 0, top: 5, bottom: 0 }}>
                ...
                <Tooltip formatter={(v) => [`${unit} ${fmtIN(v)}`, "Price"]} />
      ...
      <div className="mt-2 grid grid-cols-1 gap-4 md:grid-cols-4">
        {[ ["Farm", ladder.farm], ["Mandi", ladder.mandi], ["Wholesale", ladder.wholesale], ["Retail", ladder.retail], ].map(([label, val]) => (
          <Card key={label} title={label} subtitle="Today">
            <div className="text-xl font-semibold">{unit} {fmtIN(val)}</div>
          </Card>
        ))}
      </div>
      ...
      <div className="mt-6 flex flex-col items-center gap-2 border-t border-gray-100 pt-6 text-center text-sm text-gray-600">
        <div>
          {t("footer_cta")} <a className="font-medium text-emerald-700 underline" href="#">{t("talk_to_us")}</a> · Deck & API docs on request.
        </div>
